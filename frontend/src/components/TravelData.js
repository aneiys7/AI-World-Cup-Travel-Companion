      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {tabs.find(t => t.id === activeTab)?.data.map((item, i) => (
            <a key={i} href={item.link || item.maps_url || "#"} target="_blank" rel="noreferrer"
              className="block bg-white/5 border border-white/10 hover:border-green-500/40 rounded-xl p-4 transition-all">
